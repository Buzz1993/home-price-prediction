...
def basic_cleaning(data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # =====================================================
        # REMOVE DUPLICATE COLUMNS AFTER LOWERCASE
        # VERY IMPORTANT FIX
        # =====================================================
        ...
        if duplicate_cols:
            ...
            # keep FIRST non-null column
            for col in duplicate_cols:

                dup_df = df.loc[:, df.columns == col]

                # combine duplicate columns safely
                combined = dup_df.bfill(axis=1).iloc[:, 0]

                # remove all duplicates
                df = df.loc[:, ~df.columns.duplicated()]

                # restore merged column
                df[col] = combined

        debug_df_snapshot(df, "AFTER LOWERCASE + DUPLICATE FIX")
